...
class Board(gd.BoardGame):
    ...
    def create_game_objects(self, level=1):
        ...
        self.board.add_unit(data[1]+self.left_padding, 0, 2, 2,
                            classes.board.Letter, "", self.transp, "", 0, alpha=True)
        self.check_btn = self.board.ships[-1]
        self.check_btn.checkable = True
        self.check_btn.init_check_images(align=1, shrink=1)
        self.btn_down = False

        self.board.add_unit(data[1]+self.left_padding, 6, 2, 2, classes.board.ImgCenteredShip, "",
                            self.transp, img_src='nav_refreshd.png', alpha=True)
    ...
    def pick_half_shape(self):
        ...
        if self.direction == 0:
            ...
            while inner:
                p1 = [random.randrange(1, self.canvas_side - 1) * self.guide_scale,
                      (self.canvas_side // 2) * self.guide_scale]
                p2 = [random.randrange(1, self.canvas_side - 1) * self.guide_scale,
                      (self.canvas_side // 2) * self.guide_scale]
