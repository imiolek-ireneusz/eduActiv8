...
# This is pretty messed up - sorry trying to keep multi-language program compatible with both python2 and python3 made me do some desperately heavy things.
    ...
    def get_btns_to_hl(self, text):
        ...
        if text != " ":
            ...
            for i in range(0, 55):
                ...
                if uc.upper() in labels:
                    hl[0] = i
                    if uc == labels[3]:
                        shift = True
                    elif uc == labels[1]:
                        shift = False
                    if uc.upper() == labels[4] or uc.upper() == labels[5]:
                        alt = True
                    else:
                        alt = False
