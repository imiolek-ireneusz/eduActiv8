...
# where a word is not found it will be in brackets line so: <word>
# these will need translating before use
#
...
lang_files = ["content-es.json", "content-ca.json", "content-it.json"]
...
for lang_id in range(len(lang_files)):
    ...
    #go over all lines looking for keys and value pairs
    for each in fl:
        m = k_prog.match(each)
        if m is not None:
            key = m.group(1)
    ...
    # create new string to save in a file
    s = ""
